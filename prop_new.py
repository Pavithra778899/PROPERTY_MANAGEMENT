...
def stream_text(text: str, chunk_size: int = 4, delay: float = 0.04):
    try:
        ...
        st.error("Error streaming text: {e}")
...
def generate_explanation(query: str, is_structured: bool, results: Optional[pd.DataFrame] = None, search_results: Optional[List[str]] = None):
    if is_structured and results is not None:
        ...
        return f"This answer was derived by searching relevant documents in the Property Management module, using Snowflake Cortex to find matching information."
    else:
...
def load_verified_questions():
    try:
        # Read YAML file from Snowflake stage
        ...
        if result and result[0]["questions"]:
            ...
            st.warning("No verified questions found in YAML file.")
            return []
    except Exception as e:
        st.error(f"Failed to load verified questions from {str(e)}: {SEMANTIC_MODEL}")
        return [
    ...
    if not st.session_state.service_metadata:
        try:
            ...
            if services:
                for s in services:
                    svc_name = s["name"]
                    svc_search_col = session.sql_query(
                        f"DESC CORTEX SEARCH SERVICE {svc_name};"
                    ).collect()[0]["search_column"]
                    service_metadata.append(
                        {"name": svc_name, "content": svc_search_col}
                    )
            st.session_state.service_metadata = service_metadata
        except Exception as e:
            st.error(f"Failed to initialize Cortex Search service metadata: {str(e)}")
            st.session_state.service_metadata = [{"name": CORTEX_SEARCH_SERVICES, "content": ""}]
...
def query_cortex_search_service(query: str):
    try:
        ...
        search_col = [s["content"] for s in service_metadata
...
if not st.session_state.authenticated:
    ...
    def display_chart(df: pd.DataFrame, prefix: str = "chart", query: str = ""):
        try:
            ...
            y_col = col2.selectbox("Y axis", remaining_cols, index=0 if remaining_cols else 0, key=f"{prefix}_y")
    ...
    with st.sidebar:
        ...
        with about_container:
            ...
            st.markdown(
                """
                This application uses **Snowflake Cortex** to interpret your natural language questions and generate data insights based on the **Property Management module**. 
                Simply ask a question about your property management data to see relevant answers and visualizations.
                """
            )
        with help_container:
            ...
            with st.spinner("Generating Response..."):
                ...
                if is_greeting and original_query.lower().strip() == "hi":
                    ...
                    for chunk in stream_text(response_content):
                        response_placeholder.markdown(response_content[:response_content.find(chunk) + len(chunk)], unsafe_allow_html=True)
                    ...
                    greeting = original_query.lower().split()[0].capitalize()
                ...
                elif is_off_topic and not (is_complete or is_summarize):
                    ...
                    if sql:
                        ...
                        response_content = "No valid SQL query was generated."
                        failed_response = True
                        assistant_response["content"] = response_content
